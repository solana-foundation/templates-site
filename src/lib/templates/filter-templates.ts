...
export function filterTemplates({
  ...
  const hasFilters = filter.trim() !== '' || selectedKeywords.length > 0 || selectedSources.length > 0
  ...
  return templates.filter((l) => {
    ...
    if (filter.trim() !== '') {
      ...
      const inDisplayName = l.displayName?.toLowerCase().includes(filter.toLowerCase())
      ...
      if (!(inName || inDisplayName || inDescription || inKeywords || inUsecase)) {
        return false
      }
    }

    // Apply source filter if any
    if (selectedSources.length > 0 && !selectedSources.some((s) => l.source.id === s)) {
      return false
    }
