...
export interface TemplateUrlState {
  clear: () => Promise<void>
  isFiltered: boolean
  search: string
  filter: string
...
export function useTemplateUrlState(): TemplateUrlState {
  ...
  return {
    clear: async () => {
      await Promise.all([setSearch(''), setFilter(''), setSelectedKeywords([]), setSelectedSources([])])
    },
    isFiltered: selectedKeywords.length > 0 || selectedSources.length > 0 || filter.length > 0,
    search,
    filter,
    selectedKeywords,
    selectedSources,
